...
//            System.out.println("solution:"+s.toString());
    ...
    //get the initial test set which is used to cover the combinations specified by the user
    ArrayList<int[]> getInitialTestSet() throws Exception{
        ...
        for (Solution s : this.normalSolutions) {
            ...
            matcher = PATTERN.matcher(value);
            j = 0;
            int[] res = new int[this.parasNum];
            while (matcher.find()) {
                String[] strs = matcher.group(0).split("=");
                if (j < this.parasNum) {
                    Object obj = Outputs.getParaValue(this, strs);
                    String sValue = null;
                    if (obj instanceof String) {
                        ...
                        // Convert if needed
                        sValue = String.valueOf(num);
                    }
                    //covert to index
                    String paraName = inputParas[j].getName();
                    edu.uta.cse.fireeye.common.Parameter parameter = sut.getParam(paraName);
                    res[j] = parameter.getIndex(sValue);
                    j++;
                }
            }
            ts.add(res);
...
//        wrapper1.outputInCSVFormat (name+"_acts_TestSet_result_"+tWay+".csv");
    ...
    //
    void setCombNoWay() throws Exception {
        ArrayList<int[]> ts  = getInitialTestSet();
        int size = ts.size();
        int paraNum = inputParas.length;
        int[][] comTs = new int[size][paraNum];
        for(int i=0; i<size; i++){
            for(int j = 0 ; j < paraNum; j++){
                if(inputParas[j].getType().getDataType()==Data.NUMBER)
                    comTs[i][j] = ts.get(i)[j] + (Integer) this.inputParas[j].getType().getLb();
                else
                    comTs[i][j] = ts.get(i)[j];
            }
        }
        this.combTs  = comTs;
    }
...
//        System.out.println("combTs:"+combTs.length);
